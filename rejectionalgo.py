...
#   lambda_nu(y) = lambda_1 1_{y in A(nu)}
#                + lambda_T 1_{y in T \ A(nu)}
#                + lambda_c 1_{y outside (A(nu) union T)}
#
# Active zone :
...
#   - dans A(V_{s-}) : acceptation = 1
#   - dans T \ A(V_{s-}) : acceptation = lambda_T/lambda_1
#   - ailleurs : acceptation = lambda_c/lambda_1
#
...
# Intensité de fond des centres de caspase hors zones actives et hors T-zone
lambda_c = 0.05

# Intensité d'activation dans la T-zone quand le point n'est pas déjà dans A(V_{s-})
# Valeur à régler selon l'expérience ; elle doit vérifier lambda_c <= lambda_T <= lambda_1.
lambda_T = 0.20

# Intensité dominante / intensité dans les zones actives
lambda_1 = 0.60

if not (0.0 <= lambda_c <= lambda_T <= lambda_1):
    raise ValueError("Il faut 0 <= lambda_c <= lambda_T <= lambda_1 pour la méthode de rejection.")
...
def point_inside_T_zone(x, y):
    """
    Zone T du rapport : colonne verticale centrale d'une grille 3x3
    + bras central gauche.

    Pour W=[0,Lx]x[0,Ly] :
      - colonne centrale : Lx/3 <= x <= 2Lx/3, pour tout y ;
      - bras gauche central : 0 <= x < Lx/3 et Ly/3 <= y <= 2Ly/3.
    """
    in_vertical_column = (Lx / 3.0 <= x <= 2.0 * Lx / 3.0)
    in_left_middle_arm = (0.0 <= x < Lx / 3.0) and (Ly / 3.0 <= y <= 2.0 * Ly / 3.0)
    return in_vertical_column or in_left_middle_arm
...
for x, y, s, u, r, tau in zip(X_star, Y_star, S_star, U_star, R_star, tau_star):
    ...
    inside_active = point_inside_current_active_zone(x, y, s)
    inside_T = point_inside_T_zone(x, y)

    if inside_active:
        # Dans la zone active : intensité cible = lambda_1.
        accept = True
    elif inside_T:
        # Dans la T-zone mais hors zone active : intensité cible = lambda_T.
        accept = (u <= lambda_T / lambda_1)
    else:
        # Hors zone active et hors T-zone : intensité cible = lambda_c.
