...
L'objectif numérique principal du dépôt est de comparer plusieurs constructions de filtres particulaires et d'étudier leur convergence lorsque le nombre de particules augmente.
...
La zone active produite par les centres de caspase est
...
La zone dans laquelle une mort cellulaire peut effectivement se produire est
...
Ainsi, une cellule ne peut mourir que dans une zone activée par la caspase et non protégée par ERK.
...
& x\in A(V_{t-}^a),\\[4pt]
\lambda_{a,T},
& x\notin A(V_{t-}^a)\ \text{et}\ x\in T,\\[4pt]
\lambda_{a,c},
& \text{sinon}.
\end{cases}
$$
...
Cette relation permet d'utiliser $\lambda_{a,1}$ comme intensité dominante dans un algorithme de **thinning**.

Pour un candidat situé en $x$, la probabilité d'acceptation est
...
=
\lambda_d
\mathbf{1}_{\{x\in A(V_{t-}^a)\}}
\mathbf{1}_{\{x\notin A(V_{t-}^p)\}}.
$$

Une mort peut donc se produire uniquement :

1. à l'intérieur d'une zone active ;
2. à l'extérieur des zones de protection ERK.

Après chaque mort observée, une nouvelle zone de protection ERK est créée autour de la position de cette mort.

Les rayons des zones actives sont simulés selon

$$
R^a
\sim
\operatorname{Exp}(\beta_R^a),
$$

et les rayons des zones de protection ERK selon

$$
R^d
\sim
\operatorname{Exp}(\beta_R^d).
$$

Les temps de vie des centres actifs et des zones ERK sont gouvernés respectivement par les taux
