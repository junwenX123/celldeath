...
#pragma omp parallel for schedule(dynamic)
    ...
    std::cout << "R=" << R << "  K=" << K << "  area_points=" << grid.size()
              ...
              << "  path_loss=raw_and_normalized_integrated_symmetric_difference"
              << '\n';
    std::cout << "N k algorithm success collapse mean_true_paired mean_PF bias abs_bias "
                 ...
                 "normalized_symdiff_error normalized_symdiff_MCSE\n";
        ...
        for (int alg = 3; alg <= 5; ++alg) {
            ...
            for (int k = 0; k < K; ++k) {
                std::vector<double> paired_truth, estimates, differences,
                                    raw_symdiff_errors,
                                    normalized_symdiff_errors;
                for (int r = 0; r < R; ++r) if (out[r].ok) {
                    double trueB = data[r].Btrue[k];
                    double estimatedB = out[r].meanB[k];
                    double segment_start = k == 0 ? 0.0 : data[r].obs[k - 1].t;
                    double segment_duration = data[r].obs[k].t - segment_start;
                    assert(segment_duration > 0.0);
                    double normalized_symdiff = out[r].raw_symdiff_error[k] /
                                                (segment_duration * p.area());
                    assert(normalized_symdiff >= -1e-12 &&
                           normalized_symdiff <= 1.0 + 1e-12);
                    ...
                    normalized_symdiff_errors.push_back(normalized_symdiff);
                }
                ...
                std::cout << N << ' ' << (k + 1) << ' '
                          ...
                          << (success ? mean(normalized_symdiff_errors) : NAN) << ' '
                          << (success ? mcse(normalized_symdiff_errors) : NAN) << '\n';
            }
        }
    }
}
