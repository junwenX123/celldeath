...
results_file = sys.argv[1] if len(sys.argv) > 1 else "results_post_endpoint.txt"
output_file = sys.argv[2] if len(sys.argv) > 2 else "post_endpoint_bias.png"
...
for idx, k_val in enumerate(K_values):
    ...
    if idx >= len(K_values) - cols:
        ax.set_xlabel("N")
    if idx % cols == 0:
        ax.set_ylabel(r"Bias for $C_k=|D(X_{s_k^d}^{+})|$")
