...
df = df[df["algorithm"].isin(["A4_full", "A5_empirical_optimal"])]
...
for idx, k_val in enumerate(K_values):
    ...
    for alg in ["A4_full", "A5_empirical_optimal"]:
        sub = df_k[df_k["algorithm"] == alg].sort_values("N")
        ax.errorbar(
            sub["N"],
            sub["normalized_symdiff_error"],
            yerr=sub["normalized_symdiff_MCSE"],
            marker="o",
            label={"A4_full": "boostrap", "A5_empirical_optimal": "empirical optimal"}[alg],
            capsize=3,
        )
...
upper = (df["normalized_symdiff_error"] +
         df["normalized_symdiff_MCSE"].fillna(0.0)).max()
axes[0].set_ylim(0, min(1.0, 1.05 * upper))

handles, labels = axes[0].get_legend_handles_labels()
fig.legend(handles, labels, loc='upper center', ncol=2, bbox_to_anchor=(0.5, 0.995))
fig.supylabel("Normalized space-time symmetric-difference error")
