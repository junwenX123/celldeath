...
struct Data { std::vector<Obs> obs; std::vector<double> Ctrue; };
    ...
    while (t < p.Tmax && (int)out.obs.size() < deaths) {
        ...
        if (v < ra) {
            ...
            if (in_D(q.s, x, y)) {
                ...
                // C_k = |D(X_{s_k^d}^+)|, evaluated immediately after
                // adding the ERK disk triggered by the observed death.
                out.Ctrue.push_back(q.c.nD * z.cell_area);
            }
        } else if (v < ra + rd + rea) {
...
static FilterResult filter(const Data& d, int N, Weight type, uint64_t seed,
    ...
    for (size_t k = 0; k < d.obs.size(); ++k) {
        ...
        // Apply the observed-death update before forming the post-endpoint
        // filtering approximation pi_k^{+,N}.
        for (int i = 0; i < N; ++i) {
        ...
        // Integral of phi_D(x)=|D(x)| with respect to pi_k^{+,N}.
        double estimated_C = 0.0;
        for (int i = 0; i < N; ++i)
            estimated_C += w[i] * x[i].c.nD * z.cell_area;
        answer.push_back(estimated_C);
    }
    return {true, answer};
}
        ...
        for (int alg = 3; alg <= 5; ++alg) {
            ...
            for (int k = 0; k < K; ++k) {
                ...
                for (int r = 0; r < R; ++r) if (out[r].ok) {
                    double trueC = data[r].Ctrue[k];
                    double estimatedC = out[r].meanC[k];
                    paired_truth.push_back(trueC);
                    estimates.push_back(estimatedC);
                    differences.push_back(estimatedC - trueC);
                }
